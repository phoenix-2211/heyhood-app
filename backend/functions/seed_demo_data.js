...
async function seedData() {
  ...
  const targetWards = ["TN-CHN-170", "TN-CHN-179", "TN-VRD-001", "TN-VRD-002"];

  // Function to delete documents in a collection matching a query/filter
  const purgeCollection = async (collectionName, filterField, filterValues) => {
    ...
    if (count > 0) {
      await batch.commit();
      console.log(`  Purged ${count} docs from "${collectionName}"`);
    }
  };

  // Purge old seeded issues, wishes, notices, users
  await purgeCollection("issues", "ward_id", targetWards);
  await purgeCollection("wishes", "ward_id", targetWards);
  await purgeCollection("notices", "ward_id", targetWards);
  await purgeCollection("users", "home_ward_id", targetWards);
  await purgeCollection("emergency_services"); // Clean services to reload

  // Purge specific representative officials to overwrite cleanly
  const officialsToPurge = ["GCC-WC-170", "GCC-ZONE-170", "GCC-WC-179", "GCC-ZONE-179", "TN-BDO-VNR-VIRUDHUNAGAR", "TN-BDO-VNR-VIRUDHUNAGAR-02", "TN-MLA-170", "TN-MLA-179"];
  for (const oid of officialsToPurge) {
    await db.collection("officials").doc(oid).delete();
  }
  console.log("  Purged target officials.");
  ...
  const wards = {
    "TN-CHN-170": { name: "Adyar", district: "Chennai", state: "Tamil Nadu", zone: "Zone 13" },
    "TN-CHN-179": { name: "Velachery", district: "Chennai", state: "Tamil Nadu", zone: "Zone 13" },
    "TN-VRD-001": { name: "Virudhunagar Ward 1", district: "Virudhunagar", state: "Tamil Nadu", zone: "East" },
    "TN-VRD-002": { name: "Virudhunagar Ward 2", district: "Virudhunagar", state: "Tamil Nadu", zone: "West" }
  };
  ...
  // 4. Define 16 Issues (Casual TN issues to be polished)
  const issueTemplates = [
    ...
    },
    {
      ...
      support_count: 52,
      status: "In Progress",
      assigned_to: "TN-BDO-VNR-VIRUDHUNAGAR-02",
      assigned_role: "Block Development Officer",
      daysAgo: 7
    },
    {
  ...
  for (let i = 0; i < issueTemplates.length; i++) {
    ...
    // UNIQUE PICSUM IMAGE SEEDS - prefix with unique issue ID to guarantee completely distinct images!
    const issueId = `HH-${template.ward}-2026-${10000 + i}`;
    const photoUrl = `https://picsum.photos/seed/issue-${issueId}/800/600`;
  ...
  // UNIQUE PICSUM IMAGE SEEDS FOR WISHES
  const seededWishes = wishTemplates.map((item, idx) => {
    return {
      ...
      imageUrl: `https://picsum.photos/seed/wish-${item.id}/800/600`,
  ...
  // 6. Define 10 Emergency Services (KYH Directory)
  ...
  const noticeTemplates = [
    ...
    { ward: "TN-CHN-179", title: "Malaria Spraying Drive in Velachery", content: "Sanitation team will spray anti-mosquito fogging across Sector 1 and 2 on Friday evening. Please keep windows closed.", type: "Alert", official: "TN-MLA-179", cannot_delete: false, daysAgo: 1 },
  ...
  // 8. Define target representatives officials to show up under "Your Representatives"
  const seededOfficials = [
    {
      official_id: "GCC-WC-170",
      name: "Sarvesh Kumar",
      designation: "Ward Councillor",
      ward_id: "TN-CHN-170",
      phone: "+91-44-24430170",
      mobile: "9840123456",
      email: "councillor.ward170@gcc.tn.gov.in",
      district: "Chennai",
      state: "Tamil Nadu",
      level: "ward",
      verified: true,
      accountability_score: 103,
      issues_assigned: 0,
      issues_resolved: 1,
      issues_overdue: 0
    },
    {
      official_id: "GCC-ZONE-170",
      name: "Ravi Chandran",
      designation: "Zone Officer",
      ward_id: "TN-CHN-170",
      phone: "+91-44-24430171",
      mobile: "9840123457",
      email: "zone13.officer@gcc.tn.gov.in",
      district: "Chennai",
      state: "Tamil Nadu",
      level: "zone",
      verified: true,
    ...
    {
      official_id: "TN-MLA-170",
      name: "T. Velu",
      designation: "MLA",
      ward_id: "TN-CHN-170",
      phone: "+91-44-24430172",
      mobile: "9840123458",
      email: "mla.ward170@assembly.tn.gov.in",
      district: "Chennai",
      state: "Tamil Nadu",
      level: "constituency",
      verified: true,
    ...
    {
      official_id: "GCC-WC-179",
      name: "Meenakshi Sundaram",
      designation: "Ward Councillor",
      ward_id: "TN-CHN-179",
      phone: "+91-44-22441179",
      mobile: "9444123456",
      email: "councillor.ward179@gcc.tn.gov.in",
      district: "Chennai",
      state: "Tamil Nadu",
      level: "ward",
      verified: true,
    ...
    },
    {
      ...
      designation: "MLA",
      ward_id: "TN-CHN-179",
      phone: "+91-44-22441180",
      mobile: "9444123457",
      email: "mla.ward179@assembly.tn.gov.in",
      district: "Chennai",
      state: "Tamil Nadu",
      level: "constituency",
      verified: true,
    ...
    {
      official_id: "TN-BDO-VNR-VIRUDHUNAGAR",
      name: "S. Pandian",
      designation: "Block Development Officer",
      ward_id: "TN-VRD-001",
      phone: "+91-4562-243505",
      mobile: "9943123456",
      email: "bdo.vrd@tn.gov.in",
    ...
    {
      official_id: "TN-BDO-VNR-VIRUDHUNAGAR-02",
      name: "S. Pandian",
      designation: "Block Development Officer",
      ward_id: "TN-VRD-002",
      phone: "+91-4562-243505",
  ...
  // 9. Calculate Wards dynamic pulse score based on resolving rate & issue severity
  ...
  // Write the specific officials and wards
  await writeBatch("officials", seededOfficials, "official_id");
  await writeBatch("wards", seededWards, "ward_id");

  // Mark Seeding as Completed (with seeded_v2 flag so it tracks this new robust version)
  const statusRef = db.collection("metadata").doc("seeding_status");
  await statusRef.set({ seeded_v1: true, seeded_v2: true, seeded_at: admin.firestore.Timestamp.fromDate(now) });
  ...
  console.log(`- Officials seeded: ${seededOfficials.length}`);
  console.log(`- Wards seeded/updated: ${seededWards.length}`);
  console.log("====================================================");
}
